...
On Apple platforms, `NLContextualEmbedding` is usually the first option to try for local text embeddings. It is provided by the NaturalLanguage framework and avoids shipping a custom model with the app.

However, this package exists because the target visionOS app needed Japanese semantic search, but `NLContextualEmbedding(language: .japanese)` did not work reliably in our visionOS testing.
...
Because of that, the project needs an app-controlled fallback path: convert a known multilingual embedding model to Core ML, bundle the model and tokenizer, and run embedding inference locally on visionOS.

Compared with the system framework, this approach is heavier because the app must manage and ship model assets. In return, it gives control over:
...
Appleプラットフォームでローカルembeddingを使う場合、通常はまず NaturalLanguage framework の `NLContextualEmbedding` を検討します。OS標準APIで使えるため、独自モデルをアプリに同梱せずに済むからです。
...
このprojectでは visionOS 上で日本語のsemantic searchを行いたかったため、`NLContextualEmbedding` だけに依存する構成では不十分でした。
